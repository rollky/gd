...
async def getJkConfig(jk):
    global befor_keys,converters, cmdName, isNow, log_send, log_type, patternStr, nameList, envNameList, scriptPathList, dlDict, yanshi, envNum, jk_list, jcDict, v_today, jk_today_file, todayEnv_tmp
    v_today = time.strftime('%Y-%m-%d', time.localtime(time.time()))
    jk_today_file = f'{LOG_DIR}/bot/jk-{v_today}.txt'
    """Do some math."""
    jk_list = jk["jk"]
    cmdName = jk["cmdName"]
    converters = jk["monitor_converters"]
    befor_keys=converters.keys()
    dlDict = {}
    jcDict = {}
    todayEnv_tmp = {}
    patternStr = ''
...
async def funCX(name, scriptPath, msg, group, lable=1):
    try:
        ...
        r = result.readlines()
        if r:
            a = random.randint(0, 10) #队列检测休眠时间
...
async def converter_handler(text):
    ...
    try:
        for message in messages:
            ...
            key = key.replace('`', '').replace('*', '')
            value = value.replace('`', '').replace('*', '')
            if key not in befor_keys:
                continue
            for i in befor_keys:
                if i==key:
                    key = converters[i]["after_key"]
                    value = converters[i]["after_value"] % value
                    text = f'export {key}="{value}"'
                    logger.info(f"转换后的变量==>{text}")
                    return text
    except Exception as e:
        title = "【💥错误💥】"
        name = "文件名：" + os.path.split(__file__)[-1].split(".")[0]
